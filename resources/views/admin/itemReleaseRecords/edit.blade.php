    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.item-release-records.update", [$itemReleaseRecord->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('received_by') ? 'is-invalid' : '' }}" type="text" name="received_by" id="received_by" value="{{ old('received_by', $itemReleaseRecord->received_by) }}" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('claimed') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="claimed" id="claimed" value="1" {{ $itemReleaseRecord->claimed || old('claimed', 0) === 1 ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('date_issued') ? 'is-invalid' : '' }}" name="date_issued_id" id="date_issued_id" required>
                    @foreach($date_issueds as $id => $date_issued)
                        <option value="{{ $id }}" {{ ($itemReleaseRecord->date_issued ? $itemReleaseRecord->date_issued->id : old('date_issued_id')) == $id ? 'selected' : '' }}>{{ $date_issued }}</option>
                    @endforeach
                </select>
