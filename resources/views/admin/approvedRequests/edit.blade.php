    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.approved-requests.update", [$approvedRequest->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('requestor') ? 'is-invalid' : '' }}" name="requestor_id" id="requestor_id" required>
                    @foreach($requestors as $id => $requestor)
                        <option value="{{ $id }}" {{ ($approvedRequest->requestor ? $approvedRequest->requestor->id : old('requestor_id')) == $id ? 'selected' : '' }}>{{ $requestor }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('item') ? 'is-invalid' : '' }}" name="item_id" id="item_id" required>
                    @foreach($items as $id => $item)
                        <option value="{{ $id }}" {{ ($approvedRequest->item ? $approvedRequest->item->id : old('item_id')) == $id ? 'selected' : '' }}>{{ $item }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description_id" id="description_id">
                    @foreach($descriptions as $id => $description)
                        <option value="{{ $id }}" {{ ($approvedRequest->description ? $approvedRequest->description->id : old('description_id')) == $id ? 'selected' : '' }}>{{ $description }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('brand') ? 'is-invalid' : '' }}" name="brand_id" id="brand_id">
                    @foreach($brands as $id => $brand)
                        <option value="{{ $id }}" {{ ($approvedRequest->brand ? $approvedRequest->brand->id : old('brand_id')) == $id ? 'selected' : '' }}>{{ $brand }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('quantity') ? 'is-invalid' : '' }}" type="number" name="quantity" id="quantity" value="{{ old('quantity', $approvedRequest->quantity) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('unit') ? 'is-invalid' : '' }}" type="text" name="unit" id="unit" value="{{ old('unit', $approvedRequest->unit) }}">
                @if($errors->has('unit'))
                    <div class="invalid-feedback">
                        {{ $errors->first('unit') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number" name="price" id="price" value="{{ old('price', $approvedRequest->price) }}" step="0.01">
                @if($errors->has('price'))
                    <div class="invalid-feedback">
                        {{ $errors->first('price') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_requested') ? 'is-invalid' : '' }}" type="text" name="date_requested" id="date_requested" value="{{ old('date_requested', $approvedRequest->date_requested) }}" required>
